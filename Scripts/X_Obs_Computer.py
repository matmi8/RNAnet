...
def main():
    ...
    Obs_REAL = np.zeros((N_step, 9))
    Obs_REAL[:,0] = beta_vet

    Obs_ER = np.zeros((N_step, 9))
    Obs_ER[:,0] = beta_vet

    Obs_REAL_dev = np.zeros((N_step,8))
    Obs_ER_dev = np.zeros((N_step, 8))
	...
	### start iterating over betas...
        for i in range(N_step):
            ...
            if(ER_ON):
                m_norm, n_norm, Xmean, Xmax, Delta, rho, Sin, Sout = OF.CompObs(links_er, beta, Opt)
                Obs_ER[i,1:] += m_norm, n_norm, Xmean, Xmax, Delta, rho, Sin, Sout
                Obs_ER_dev[i,:] += m_norm**2, n_norm**2, Xmean**2, Xmax**2, Delta**2,  rho**2, Sin**2, Sout**2

            if(REAL_ON):
                m_norm, n_norm, Xmean, Xmax, Delta, rho, Sin, Sout = OF.CompObs(links, beta, Opt)
                Obs_REAL[i,1:] += m_norm, n_norm, Xmean, Xmax, Delta, rho, Sin, Sout
		Obs_REAL_dev[i,:] += m_norm**2, n_norm**2, Xmean**2, Xmax**2, Delta**2, rho**2, Sin**2, Sout**2
        ...
        if(ER_ON):
            ...
            np.savetxt(savefile, Res, header = "beta m_norm, n_norm, Xmean, Xmax, Delta, rho, Sin, Sout, dev_m_norm, dev_n_norm, dev_Xmean, dev_Xmax, dev_Delta, dev_rho, dev_Sin, dev_Sout")
